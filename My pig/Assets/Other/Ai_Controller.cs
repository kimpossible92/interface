...
public class Ai_Controller : Controller
{
        ...
        switch (state)
        {
            ...
            case AI_STATES.IDLE:
                ...
                if (dodge)
                {
                    ...
                    {// Detect if standing on a bomb instead
    ...
    private ArrayList calculate_path_to(Vector3 startpos, Vector3 goal)
    {
        ...
        Map temp = FindObjectOfType<GGC>().map;
        ...
        while (!done)
        {
            ...
            foreach (dir_dist d in get_closest_collisions(currentpos))
            {
                ...
                if (temp.is_walkable((int)temp_pos.x, (int)temp_pos.y))
                {
                    if (Math.Abs(goal.x - temp_pos.x) < Math.Abs(goal.x - currentpos.x) ||
                    Math.Abs(goal.y - temp_pos.y) < Math.Abs(goal.y - currentpos.y))
                    {
    ...
    private Vector3 get_random_walkable_node_position()
    {
        ...
        bool found = false;
        Map m = FindObjectOfType<GGC>().map;
