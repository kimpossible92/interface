...
public class Map : MonoBehaviour
{
    ...
    public void construct(int _start_poses, int x, int y, GameObject parent)
    {
        ...
        startpos_prefab = (GameObject)Resources.Load("sp", typeof(GameObject));
    ...
    private void create_map(int x, int y)
    {
        ...
        // player start pos
        GameObject t = new_instance(1, y / 2 + (y % 2), 0, startpos_prefab);
        ...
        if (start_poses > 1)
        {
            new_instance(1, y - 2, 0, startpos_prefab);
            array_representation[1, y - 2] = Blocks.Startpos;

            if (start_poses > 2)
            {
                //add four more
                new_instance(1, 1, 0, startpos_prefab);
                array_representation[1, 1] = Blocks.Startpos;

                if (start_poses > 3)
                {
                    new_instance(x - 2, y - 2, 0, startpos_prefab);
                    array_representation[x - 2, y - 2] = Blocks.Startpos;
                    if (start_poses > 4)
                    {
                        new_instance(x - 2, 1, 0, startpos_prefab);
        ...
        for (int i_x = 0; i_x < x; i_x++)
        {
            for (int i_y = 0; i_y < y; i_y++)
            {
                ...
                if (i_x == 0 || i_x == x - 1 || i_y == 0 || i_y == y - 1)
                {
                    ...
                    //    new_instance(i_x, i_y, 0, wall_prefab);
                ...
                else
                {
                    ...
                    else
                    {
                        if (!start_next_to(i_x, i_y))
                        {
                            ...
                            new_instance(i_x, i_y, 0, breakable_prefab);
                        }
                    }
                }
            }
